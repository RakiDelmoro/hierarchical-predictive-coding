...
```
  ...
  (target)          │  ┌─────────────────────────────────┐    │
                    ...
                    │  └─────────────────────────────────┘    │
                    │              ↑                           │
  Frame t ────────→ ENCODER ──→ z1 (16×16) ──→ DECODER ──→ Prediction
                    (compress)    └─── z2 (8×8) ──┘
                                  (two-level mind)
```
...
```
  64×64 image (4,096 values) ──→ 16×16×128 latent (32,768 values)
```
...
### Latent Hierarchy — "The Two-Layer Mind"
```
  z1 (16×16×128) = FINE DETAILS     │  z2 (8×8×128) = BIG PICTURE
  ─────────────────────────────────  │  ─────────────────────────────────
  • Exact edge positions             │  • Motion direction
  • Textures, shapes                 │  • Speed of movement
  • Where things are                 │  • Where things are going
  Like reading individual letters    │  Like understanding the plot
```
**Why two layers:** z1 sees WHAT (details), z2 sees HOW (dynamics). Together they capture both.
**Communication:** z2 predicts what z1 should look like. z1 feeds details back to z2. They talk to refine each other.

### Decoder — "The Artist"
```
  16×16×128 latent ──→ 64×64 image
```
...
```
  ┌─────────────────────────────────────┐
  │ INPUT:                              │
  │ • Current latents z1, z2 (our guess)│
  │ • Errors (what went wrong)          │
  │ • Frames (context)                  │
  │                              ↓       │
  │ NEURAL NETWORK (2 layers, CNN)      │
  │                              ↓       │
  │ OUTPUT: How to fix z1 and z2        │
  └─────────────────────────────────────┘
```
...
```
  ...
  Step 3: Ask predictor how to fix
  Step 4: Update z1, z2
...
**Latent2 Error:** ||z2 - transition(z2)||²
...
**Model:** ~3.07M parameters | **Input:** 64×64 grayscale | **Latent:** z1(16×16×128), z2(8×8×128)
...
INPUT: Frame t (digit "0" at top-right) + Frame t+1 (digit "0" moved down-right)
  ...
  1. ENCODE: Frame t → z1 + z2 (compresses scene + dynamics)
